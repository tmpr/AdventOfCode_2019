...
class TestRelativeIntComp:
    ...
    def test_copy_of_itself(self):
        ...
        assert (list(computer_1.run_program(input_instruction=None)) 
    ...
    def test_return_16_digit_number(self):
        ...
        assert len(str(list(computer.run_program(input_instruction=None))[0])) == 16
    
    def test_large_number_in_middle(self):
        intcode = "104,1125899906842624,99"
        computer = IntComputer(intcode)
        assert list(computer.run_program(input_instruction=None))[0] == 1125899906842624