...
class Account:
    ...
    def parse_recipes(self, input_recipe: str):
        ...
        individual_recipes = [recipe.split("=>") for recipe in individual_recipes]
...
class Recipe:
    ...
    def __init__(self, cost: list, product_: str, account_object: Account):
        ...
        self.costs = {summand.split()[1] : int(summand.split()[0]) for summand in cost}
...
def compute_ore_cost(input_recipe: str) -> int:
    ...
    while any([my_account.debt[thing] > 0 for thing in my_account.debt.keys() if thing != "ORE"]):
...
def compute_possible_fuel_output(number_of_ORE, input_recipe):
    needed_for_one_fuel = compute_ore_cost(input_recipe)
    return number_of_ORE // needed_for_one_fuel

if __name__ == "__main__":
    with open("my_input.txt", "r") as f:
        my_input = f.read()
    print(compute_ore_cost(my_input))
    print(compute_possible_fuel_output(1_000_000_000, my_input) < 3000)